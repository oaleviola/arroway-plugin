...
**Provenance is the rule that matters most here.** Set `decided_by_human=true` **only** when the person stated or sanctioned it in this conversation. Your own inference is saved as a *proposal* for their review, which is the normal case and costs nothing. A proposal never presents itself as a decision.
