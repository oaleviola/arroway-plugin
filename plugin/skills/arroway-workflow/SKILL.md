...
**Three things fail that test however true they are.** An instruction to check, verify, be careful or look at the source — the next assistant is not worse at its job for lacking it. A step of a routine: that belongs in the prompt that runs the routine, not in what every session reads. And anything that is here only because something went wrong once — that is a scar, not a norm. What does not deserve Arroway does not deserve anywhere else either, so leaving it out is the decision, not the omission.

**What passes is narrower than it looks, and easy to walk past.** A fact about the system or the person that no model derives by reading the code, the data or the task — which table is canonical, which column is a backfill stamp, which check is a real build and which is a skip. A decision that could have gone the other way. And who someone is. Those are worth the person's review; the three above are not.

**Know what it costs before you write it.** Only a **pinned** memory travels in every read of this project from now on. A normal memory competes for space in the block relevant to the task at hand — it comes back when it matches the work, and not otherwise. And before any of that, every proposal costs the person a review. Nothing pins itself: a `rule` is written in **two calls** — the first returns the memories already here, whether each of them is pinned, and what the fixed block costs today; the second carries the server's token and your pin decision, made against what you just read. Three questions decide it: does breaking it cause expensive or irreversible damage · does it apply to every task, or only when someone touches one area · would the next person have found it anyway when they opened the relevant file? A rule that is discoverable where it matters is a reference or a fact. And a pin does not push another pin out — it eats the sampled space of every read, which is where someone else's task finds what it needs.

**The person's order and your judgement are different things.** `pin_suggested` is what *you* concluded, and above the block's budget the server turns it into a proposal of the pin alone — the memory still lands and works. `pin_requested_by_human` is only for when they actually said to fix it in every session, and that pins straight away. Agreeing the content is right is not asking for the pin. Unpinning is never yours: it is their gesture, in the panel.
...
**A finding costs nobody a review.** A source-backed fact or reference that another agent can reopen and compare goes in with `treatment=finding`, and it skips the human queue entirely — for context that is the right route, and the cheap one. What it gives up is force. Findings are context, never a norm: it cannot be pinned, cannot replace a sanctioned memory, and never justifies an action a rule forbids. If a person revoked a finding, do not write it again unless they reconsidered in this conversation — a flag on the related-memories check does not unlock that. Correcting another finding is in-place replacement, not a proposal.
